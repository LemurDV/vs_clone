...
def draw_hud(screen, player_stats, wave_info, show_level_up=False):
    ...
    # Фон для HUD
    pygame.draw.rect(screen, (30, 30, 30, 180), (0, 0, WIDTH, 120),
                     border_radius=10)
    pygame.draw.rect(screen, (60, 60, 60), (0, 0, WIDTH, 120), 2,
                     border_radius=10)
...
def draw_player_stats(screen, stats, x, y):
    ...
    level_text = FONT_SMALL.render(
        f"Уровень: {stats['level']}", True, WHITE)
    ...
    damage_text = FONT_SMALL.render(
        f"Урон: {stats['damage']}", True, WHITE)
...
def draw_wave_info(screen, wave_info, x, y):
    ...
    wave_text = FONT_SMALL.render(f"Волна: {wave_info['current_wave']}", True,
                                  WHITE)
    ...
    if wave_info['total_enemies'] > 0:
        ...
        pygame.draw.rect(screen, GREEN,
                         (x, y + 55, bar_width * progress, bar_height))
...
def draw_level_up_message(screen):
    ...
    screen.blit(s,
                (WIDTH // 2 - message.get_width() // 2 - 20, HEIGHT // 2 - 100))
    screen.blit(message,
                (WIDTH // 2 - message.get_width() // 2, HEIGHT // 2 - 90))
