...
class UpgradeMenu:
    ...
    def show(self, upgrades):
        """Показать меню с указанными улучшениями"""
        self.active = True
        self.upgrade_options = upgrades[:3]  # Берем первые 3 улучшения
        self.selected_option = 0
        self.hovered_option = -1
    ...
    def handle_keydown(self, key):
        ...
        if key == pygame.K_UP:
            ...
            self.selected_option = min(
                len(self.upgrade_options) - 1, self.selected_option + 1
            )

        # Выбор с помощью клавиш 1-3 или Enter
        if key == pygame.K_RETURN:
            self.select_option(self.selected_option)
        elif key == pygame.K_1 and len(self.upgrade_options) > 0:
            self.select_option(0)
        elif key == pygame.K_2 and len(self.upgrade_options) > 1:
            self.select_option(1)
        elif key == pygame.K_3 and len(self.upgrade_options) > 2:
    ...
    def draw(self, screen):
        """Отрисовка меню"""
        if not self.active or not self.upgrade_options:
            return
        ...
        # Заголовок
        title = self.font.render("ВЫБЕРИТЕ УЛУЧШЕНИЕ", True, YELLOW)
