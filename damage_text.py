...
# damage_text.py - улучшенная позиция
class DamageText:
    def __init__(self, x, y, damage, is_crit=False):
        self.x = x
        self.y = y - 20  # Поднимаем выше врага
        self.damage = str(int(damage))
        self.is_crit = is_crit
        self.lifetime = 1000  # мс
        self.created_time = pygame.time.get_ticks()
        self.velocity_y = -1.5  # Всплывание вверх
    ...
    def update(self):
        ...
        self.velocity_y += 0.05  # Гравитация
    ...
    def draw(self, screen):
        ...
        # Прозрачность (исчезание)
        alpha = 255 * (1 - age / self.lifetime)

        # Размер шрифта (для крита - больше)
        if self.is_crit:
            font_size = 28
